	...
	public class BakeAnimation : MonoBehaviour
	{
		...
		public AnimationClip[] Run()
		{
			...
			for (int j = 0; j < animClips.Count; j++)
			{
				...
				for (int i = 0; i < numFramesInAnim + 1; i++)
				{
					...
					transform.GetComponent<GlobalControl>().Run();
		...
		List<Transform> GetListBones()
		{
			...
			foreach (GameObject child in childsOfGameobject)
			{
				...
				{
					SkinnedMeshRenderer _smr = child.transform.GetComponent<SkinnedMeshRenderer>();
					
					foreach (Transform bone in _smr.bones)
						returnList.Add(bone);
		...
		void bakeAnimation(Transform bone, float time)
		{
			...
			//boneNameFul = boneNameFul.Replace("_JNT", "");
			...
			if (time > 0)
			{
				//if (time == frameTime || (curvePosX[index].Evaluate(time - frameTime) != bone.localPosition.x) ||
			...
			if (time > 0)
			{
				//if (time == frameTime || (curveRotX[index].Evaluate(time - frameTime) != bone.localRotation.x) ||
			...
			if (time > 0)
			{
				//if (time == frameTime || (curveScaleX[index].Evaluate(time - frameTime) != bone.localScale.x) ||
		...
		{
			if (!Directory.Exists(_puppet3DPath + "/Animation/Baked"))
			{
				AssetDatabase.CreateFolder(_puppet3DPath + "/Animation", "Baked");
				AssetDatabase.Refresh();
			}
			string path = AssetDatabase.GenerateUniqueAssetPath(_puppet3DPath + "/Animation/Baked/" + a.name + ".asset");
