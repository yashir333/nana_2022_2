...
namespace Puppet3D
{
	...
	{		
		...
		void Awake()
		{
			...
			//intialize all IKFK blends
			foreach (var blend in _IKFKBlends)
			{
				blend.GlobalControl = this;
				blend.Init();
			}
			
			//return to last know position after initialization
		...
		public void Refresh(bool ikfkblends = true)
		{
			...
			_DrivenKeys.Clear();
			if (ikfkblends) _IKFKBlends.Clear();
			_Controls.Clear();
			_Bones.Clear();
		...
		private void FaceCamera()
		{
			for (int i = 0; i < _IkhandlesArray.Length; ++i)
			{
				_IkhandlesArray[i].AimDirection = transform.forward.normalized * _flipCorrection;
			}
		}
	}	
