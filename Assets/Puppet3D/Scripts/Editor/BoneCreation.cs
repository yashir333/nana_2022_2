...
namespace Puppet3D
{
	public class BoneCreation : Editor {
		...
		public static void MirrorBones(string ReplaceFrom, string ReplaceTo, bool leftToRight)
		{
			GameObject selectedBone = Selection.activeGameObject;
			if (selectedBone != null)
			{
				Bone selectedBoneComp = selectedBone.GetComponent<Bone>();

				GameObject parentGO = selectedBone.transform.parent.gameObject;
				//Bone[] bones = selectedBone.transform.GetComponentsInChildren<Bone>();
				...
				Vector3 mirroredPos = selectedBone.transform.position;
				mirroredPos.x *= -1f;
				GameObject mirroredBones = BoneCreationMode(mirroredPos, new Ray(), true);
				mirroredBones.GetComponent<Bone>().Radius = selectedBoneComp.Radius;
				string newName = selectedBone.name;

				if(leftToRight)
					newName = newName.Replace(ReplaceFrom, ReplaceTo);
				else
					newName = newName.Replace(ReplaceTo, ReplaceFrom);

				mirroredBones.name = newName;

				foreach (Transform child in selectedBone.transform)
				{
					MiirorAllChildBones(bones, child, leftToRight, ReplaceFrom, ReplaceTo);
				}
				/*
				foreach (Bone bone in bones)
				{
					if (bone != selectedBoneComp)
					{
						mirroredPos = bone.transform.position;
						mirroredPos.x *= -1f;
						mirroredBones = BoneCreationMode(mirroredPos, new Ray(), true);
						mirroredBones.GetComponent<Bone>().Radius = selectedBoneComp.Radius;
						newName = bone.name;
						if (leftToRight)
								newName = newName.Replace(ReplaceFrom, ReplaceTo);
				...
				BoneFinishCreation();
				Selection.activeGameObject = mirroredBones;
			}
		}
		public static List<Bone> MiirorAllChildBones(List<Bone> bones, Transform parentTrans, bool leftToRight, string ReplaceFrom, string ReplaceTo)
		{
			Vector3 mirroredPos;
			GameObject mirroredBones;
			string newName;
			Bone bone = parentTrans.GetComponent<Bone>();
			if (bone != null)
			{
				...
				mirroredPos.x *= -1f;
				mirroredBones = BoneCreationMode(mirroredPos, new Ray(), true);
				mirroredBones.GetComponent<Bone>().Radius = bone.Radius;
				newName = bone.name;
				if (leftToRight)
					newName = newName.Replace(ReplaceFrom, ReplaceTo);
				else
