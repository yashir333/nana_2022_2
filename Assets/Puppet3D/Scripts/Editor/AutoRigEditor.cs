...
namespace Puppet3D
{
	public class AutoRigEditor : Editor
	{
		...
		public static void MakeGuidesMenu()
		{
			MakeGuides();
		}
		...
		public static void CreateModRig(bool rigFingers = false)
		{
			...
			var initPos = anim.transform.position;
			var initRot = anim.transform.rotation;
			
			//zero out before setup
			...
			if (Chest == null)
			{
				Debug.LogWarning("Needs an Avatar with Chest bone"); return;
			}
			else if (Chest.childCount == 1)
			{
				Chest = Chest.GetChild(0);
			}
			...
			for (int i = 0; i < children.Length; i++)
			{
				...
				//part of fix, record serializable global control component
				ikfkBlend.GlobalControl = globalControl;
			...
			*/
			...
			//return to last know position after
			anim.transform.position = initPos;
			anim.transform.rotation = initRot;

			hips.gameObject.SetActive(false);
			//globalControl.transform.Find("Spine_01").gameObject.SetActive(false);

			if (hips.name == "Spine_01_JNT")
			{
				anim.transform.Find("Spine_01").name = "Spine_01_P3D";
			}
		...
		public static void AutoRigMenu()
		{
			AutoRig();
		}
