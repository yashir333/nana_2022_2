...
namespace Puppet3D
{
	public class ColladaExporter : IDisposable
	{
		...
		public void Save()
		{
			string invariantString = this.path.ToString(CultureInfo.InvariantCulture);
			this.xml.Save(invariantString);
		}
		...
		public float[] SampleAnimationTimesForExport(AnimationClip clip, GameObject boneGO, GameObject rootGO)
		{
			...
			{	
				timeVals.Add(i * frameTime);
			}
			return timeVals.ToArray();

		}
		public Matrix4x4[] SampleAnimationForExport(AnimationClip clip, GameObject boneGO, GameObject rootGO)
		{
			List<Matrix4x4> bonePositions = new List<Matrix4x4>();
			...
			for (int i = 0; i < numFramesInAnim + 1; i++)
			{
				...
				AnimationMode.SampleAnimationClip(rootGO, clip, i * frameTime);

				//Debug.Log(clip.name + " " + boneGO.name + " " + (i * frameTime) + " " + boneGO.transform.localPosition);
				Matrix4x4 mat = GetMatrixVal(boneGO.transform);

				bonePositions.Add(mat);
				//clip.SampleAnimation(boneGO, i * frameTime);
				
			}
			AnimationMode.EndSampling();
			AnimationMode.StopAnimationMode();
			return bonePositions.ToArray();

		}
		public XmlNode AddAnimation(Transform bone, GameObject rootGO, AnimationClip clip)
		{
			string id = bone.name;
			XmlNode animation = this.animations.AppendChild(this.xml.CreateElement("animation", COLLADA));
			XmlNode anim = animation.AppendChild(this.xml.CreateElement("animation", COLLADA));
			XmlNode nodeA, nodeB, nodeC, nodeD;
			XmlAttribute attr;
			//StringBuilder str;
			...
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-anim";
			animation.Attributes.Append(attr);
			...
			nodeA = anim.AppendChild(this.xml.CreateElement("source", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Matrix-animation-input";
			nodeA.Attributes.Append(attr);

			nodeB = nodeA.AppendChild(this.xml.CreateElement("float_array", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Matrix-animation-input-array";
			nodeB.Attributes.Append(attr);
			attr = this.xml.CreateAttribute("count");

			//Animator animator = bone.GetComponentInParent<Animator>();
			//AnimationClip[] animClips = GetAllAnimationClips(animator);
			float[] timeVals = SampleAnimationTimesForExport(clip, bone.gameObject, rootGO);
			string timeValString = "";
			foreach (var m in timeVals)
			{
				timeValString += m + " ";
			}
			attr.Value = timeVals.Length.ToString();
			nodeB.Attributes.Append(attr);

						
			nodeB.AppendChild(this.xml.CreateTextNode(timeValString));

			//str = null;

			var matrixes = SampleAnimationForExport(clip, bone.gameObject, rootGO);
			string matrixString = "";
			...
			nodeC.Attributes.Append(attr);
			attr = this.xml.CreateAttribute("count");
			attr.Value = matrixes.Length.ToString();
			nodeC.Attributes.Append(attr);
			...
			nodeA = anim.AppendChild(this.xml.CreateElement("source", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Matrix-animation-output-transform";
			nodeA.Attributes.Append(attr);

			nodeB = nodeA.AppendChild(this.xml.CreateElement("float_array", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Matrix-animation-output-transform-array";
			nodeB.Attributes.Append(attr);
			...
			attr.Value = matrixes.Length.ToString();
			nodeB.Attributes.Append(attr);
			nodeB.AppendChild(this.xml.CreateTextNode(matrixString));
			...
			nodeA = anim.AppendChild(this.xml.CreateElement("source", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Interpolations";
			nodeA.Attributes.Append(attr);

			nodeB = nodeA.AppendChild(this.xml.CreateElement("Name_array", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Interpolations-array";
			nodeB.Attributes.Append(attr);
			attr = this.xml.CreateAttribute("count");
			attr.Value = matrixes.Length.ToString();
			nodeB.Attributes.Append(attr);
			...
			{
				if (i != timeVals.Length - 1)
				{
					interpolationStrings += "LINEAR ";
				}
				else
				{
					interpolationStrings += "LINEAR";
			...
			nodeB.AppendChild(this.xml.CreateTextNode(interpolationStrings));
			nodeB = nodeA.AppendChild(this.xml.CreateElement("technique_common", COLLADA));
			nodeC = nodeB.AppendChild(this.xml.CreateElement("accessor", COLLADA));
			attr = this.xml.CreateAttribute("source");
			attr.Value = "#" + id + "-Interpolations-array";
			...
			nodeA = anim.AppendChild(this.xml.CreateElement("sampler", COLLADA));
			attr = this.xml.CreateAttribute("id");
			attr.Value = id + "-Matrix-animation-transform";
			nodeA.Attributes.Append(attr);
			...
			nodeA = anim.AppendChild(this.xml.CreateElement("channel", COLLADA));
			attr = this.xml.CreateAttribute("source");
			attr.Value = "#" + id + "-Matrix-animation-transform";
			nodeA.Attributes.Append(attr);
			attr = this.xml.CreateAttribute("target");
			attr.Value = id + "/matrix";
			nodeA.Attributes.Append(attr);

			return animation;
		}
		public static AnimationClip[] GetAllAnimationClips(Animator animator)
		{
			...
			for (int i = 0; i < controller.animationClips.Length; i++)
			{
				...
				string path = UnityEditor.AssetDatabase.GetAssetPath(controller.animationClips[i]);
				clip = (AnimationClip)UnityEditor.AssetDatabase.LoadAssetAtPath(path, typeof(AnimationClip));
		...
		public void AddControllerToScene(Transform[] bones, AnimationClip clip = null, GameObject rootGO = null)
		{
			for (int i = 0; i < bones.Length; i++)
			{
				...
				if (parentIndex < 0)
				{
					AddControllerToSceneRecursive(bones[i], null, bones, clip, rootGO);
				}

			}

		}
		public Matrix4x4 GetMatrixVal(Transform bone)
		{
		...
		public void AddControllerToSceneRecursive(Transform bone, XmlNode ParentNode, Transform[] bones, AnimationClip clip = null, GameObject rootGO = null )
		{
			if(clip!=null && rootGO!=null)
				AddAnimation(bone, rootGO, clip);

			Matrix4x4 mat = GetMatrixVal(bone);
			...
			foreach (Transform child in bone.transform)
			{
				if (HasThis(child, bones))
					AddControllerToSceneRecursive(child, ParentNode, bones, clip, rootGO);
			}
