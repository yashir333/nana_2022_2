	...
	public class Skinning : Editor
	{
		...
		public static GameObject BindSmoothSkin(int isGeosedic = 1)
		{
			...
			foreach (GameObject Obj in selection)
			{
				...
				{
					if (isGeosedic == 0 && Obj.GetComponent<MeshFilter>() != null && Obj.GetComponent<MeshFilter>().sharedMesh.vertexCount > 25000)
					{
						bool check = EditorUtility.DisplayDialog(Obj.name + " has quite a few polys", "Which skinning algorithm do you want to use?", "Voxel(Slow)", "ClosestPoint (Fast)");
						if (!askedQuestion && !check)
						{
								isGeosedic = 1;
							askedQuestion = true;
						}
			...
			if (selectedMeshes.Count > 2)
			{
				if (isGeosedic == 0)
				{
					if (!askedQuestion && !EditorUtility.DisplayDialog("You're skinning multiple models", "Which skinning algorithm do you want to use?", "Voxel(Slow)", "ClosestPoint (Fast)"))
						isGeosedic = 1;
				}
			}
			...
			{
				// CHECK IF THERE ARE BONES INSIDE THE BOUNDS
				Bounds bounds = selectedMeshes[ind].GetComponent<Renderer>().bounds;
				bool hasBoneInsideBounds = false;
				foreach (Transform b in selectedBones)
				{
					if (bounds.Contains(b.position))
					{
						hasBoneInsideBounds = true;
						break;
					}
				}
				if (!hasBoneInsideBounds)
					isGeosedic = 1;
		...
		static void GeosedicSkinWeights(GameObject mesh, List<Transform> selectedBones)
		{
			...
			while (outsideVoxelQueue.Count > 0)
			{
				...
				int counter = 0;// 2147483647;

				foreach (Vector3 neighbourIndex in voxel.NeighbourIndexes)
				{
					Voxel neighbour = voxels[(int)neighbourIndex.x, (int)neighbourIndex.y, (int)neighbourIndex.z];

					if (!voxel.Set && neighbour != null && !neighbour.Inside && neighbour.Bone0 == -1)
					{
						neighbour.Bone0 = voxel.Bone0;
						neighbour.Weight0 = voxel.Weight0;
						neighbour.Bone1 = voxel.Bone1;
						neighbour.Weight1 = voxel.Weight1;
						counter++;
						//if (outsideVoxelQueue.Count < 99999)
							outsideVoxelQueue.Enqueue(neighbour);
					}
		...
		public static Mesh FreezeMesh(Mesh mesh, Transform model)
		{
			Mesh newMesh = new Mesh();
			Vector3[] verts = mesh.vertices;
			for (int i = 0; i < mesh.vertexCount; i++)
			{
				verts[i] = model.TransformPoint(verts[i]);
			}
		...
		public static void TransferSkinWeights(Transform source, Transform dest)
		{
			Dictionary<float, Vector3> distDic = new Dictionary<float, Vector3>();

			SkinnedMeshRenderer smrSource = source.GetComponent<SkinnedMeshRenderer>();
			SkinnedMeshRenderer smrDestination = dest.GetComponent<SkinnedMeshRenderer>();
			if (smrSource == null)
			{
				Debug.LogWarning("The first selected model (Source) should have a SkinnedMeshRenderer");
				return;
			}

			if (smrDestination == null)
			{
				MeshFilter mf = dest.GetComponent<MeshFilter>();
				//mf.sharedMesh = FreezeMesh(mf.sharedMesh, dest);
				mf.sharedMesh = SaveFBXMesh(mf.sharedMesh, true);
				Object[] objs = new Object[2];
				objs[0] = smrSource.bones[0].gameObject;
				objs[1] = dest.gameObject;
				Selection.objects = objs;
				BindSmoothSkin(1);
				smrDestination = dest.GetComponent<SkinnedMeshRenderer>();
			}
			smrDestination.quality = SkinQuality.Bone4;

			Mesh meshSource = smrSource.sharedMesh;
			// Populate array for testing purpose - use your array of objects
			Mesh meshDestination = smrDestination.sharedMesh;// FreezeMesh(smrDestination.sharedMesh, dest);
			
			Vector3[] vertsToDeform = meshDestination.vertices;
			BoneWeight[] bwsToDeform = meshDestination.boneWeights;
			//Matrix4x4[] bpsToDeform = meshDestination.bindposes;

			Vector3[] verts = meshSource.vertices;
			int[] tris = meshSource.triangles;
			BoneWeight[] bws = meshSource.boneWeights;

			//Transform t = smrSource.transform;
			//BoneWeight b = new BoneWeight();
			for (int j = 0; j < vertsToDeform.Length; j++)
			{
				if(j%100 == 0)
					EditorUtility.DisplayProgressBar("Transfering", "", (float)j / vertsToDeform.Length);

				distDic.Clear();
				for (int i = 0; i < tris.Length; i+=3)
				{
					//float dist = Vector3.Distance(dest.TransformPoint(vertsToDeform[j]), source.TransformPoint(verts[i]));
					Vector3 posToCompare = (verts[tris[i]] + verts[tris[i+1]] + verts[tris[i+2]]) / 3f;
					posToCompare = source.TransformPoint(posToCompare);
					float dist = Vector3.Distance(dest.TransformPoint(vertsToDeform[j]), posToCompare);
					if (!distDic.ContainsKey(dist))
						distDic.Add(dist, new Vector3((float)tris[i], (float)tris[i+1], (float)tris[i+2]));
				}
				...
				Vector3 nearestIndexes = distDic[distances[0]];
				Vector3 bary = BarycentricCoords(verts[(int)nearestIndexes[0]], verts[(int)nearestIndexes[1]], verts[(int)nearestIndexes[2]], vertsToDeform[j]);

				BoneWeight bw = BarycentricWeights(bws[(int)nearestIndexes[0]], bws[(int)nearestIndexes[1]], bws[(int)nearestIndexes[2]], bary);
				bwsToDeform[j] = bw;

			}
			List<Matrix4x4> newBindPoses = meshSource.bindposes.ToList();
			//Matrix4x4[] newBindPoses = meshSource.bindposes;
			for (int i = 0; i < meshSource.bindposes.Length; i++)
			{
				newBindPoses[i] = smrSource.bones[i].worldToLocalMatrix * dest.localToWorldMatrix;
			}
			List<Transform> newBones = smrSource.bones.ToList();
			for (int i = 0; i < smrDestination.bones.Length; i++)
			{
				if (!newBones.Contains(smrDestination.bones[i]))
				{
					newBones.Add(smrDestination.bones[i]);
					newBindPoses.Add(meshDestination.bindposes[i]);
				}
			}
			//m.vertices = vertsToDeform;		
			meshDestination.boneWeights = bwsToDeform;
			smrDestination.bones = newBones.ToArray();
			meshDestination.bindposes = newBindPoses.ToArray();
			smrDestination.sharedMesh = meshDestination;

			EditorUtility.ClearProgressBar() ;

			// Do something with furthestObj
		}
		public static BoneWeight BarycentricWeights(BoneWeight b1, BoneWeight b2, BoneWeight b3, Vector3 bary)
		{
			int[] b1Bones = new int[4] { b1.boneIndex0, b1.boneIndex1, b1.boneIndex2, b1.boneIndex3 };
			float[] b1Weights = new float[4] { b1.weight0, b1.weight1, b1.weight2, b1.weight3 };
			int[] b2Bones = new int[4] { b2.boneIndex0, b2.boneIndex1, b2.boneIndex2, b2.boneIndex3 };
			float[] b2Weights = new float[4] { b2.weight0, b2.weight1, b2.weight2, b2.weight3 };
			int[] b3Bones = new int[4] { b3.boneIndex0, b3.boneIndex1, b3.boneIndex2, b3.boneIndex3 };
			float[] b3Weights = new float[4] { b3.weight0, b3.weight1, b3.weight2, b3.weight3 };
			

			List<int> uniqueBones = new List<int>();
			List<float> Weights = new List<float>();

			for (int i = 0; i < b1Bones.Length; i++)
			{
				if (!uniqueBones.Contains(b1Bones[i]))
				{
					uniqueBones.Add(b1Bones[i]);
					Weights.Add(0f);
				}
			}
			for (int i = 0; i < b2Bones.Length; i++)
			{
				if (!uniqueBones.Contains(b2Bones[i]))
				{
					uniqueBones.Add(b2Bones[i]);
					Weights.Add(0f);
				}
			}
			for (int i = 0; i < b3Bones.Length; i++)
			{
				if (!uniqueBones.Contains(b3Bones[i]))
				{
					uniqueBones.Add(b3Bones[i]);
					Weights.Add(0f);
				}
			}

			List<int> uniqueBonesReduced = new List<int>();
			List<float> WeightsReduced = new List<float>();


			for (int j = 0; j < uniqueBones.Count; j++)
			{
				for (int i = 0; i < b1Bones.Length; i++)
				{
					if (b1Bones[i] == uniqueBones[j])
					{
						Weights[j] += bary.x * b1Weights[i];
						break;
					}
				}
				for (int i = 0; i < b1Bones.Length; i++)
				{
					if (b2Bones[i] == uniqueBones[j])
					{
						Weights[j] += bary.y * b2Weights[i];
						break;
					}
				}
				for (int i = 0; i < b3Bones.Length; i++)
				{
					if (b3Bones[i] == uniqueBones[j])
					{
						Weights[j] += bary.z * b3Weights[i];
						break;
					}
				}
				if (Weights[j] > 0)
				{
					uniqueBonesReduced.Add(uniqueBones[j]);
					WeightsReduced.Add(Weights[j]);
				}
			}
			
			BoneWeight baryWeight = new BoneWeight();
			if (uniqueBonesReduced.Count > 0)
			{
				baryWeight.boneIndex0 = uniqueBonesReduced[0];
				baryWeight.weight0 = WeightsReduced[0];
			}
			if (uniqueBonesReduced.Count > 1)
			{
				baryWeight.boneIndex1 = uniqueBonesReduced[1];
				baryWeight.weight1 = WeightsReduced[1];
			}
			if (uniqueBonesReduced.Count > 2)
			{
				baryWeight.boneIndex2 = uniqueBonesReduced[2];
				baryWeight.weight2 = WeightsReduced[2];
			}
			if (uniqueBonesReduced.Count > 3)
			{
				baryWeight.boneIndex3 = uniqueBonesReduced[3];
				baryWeight.weight3 = WeightsReduced[3];
			}
			float totalWeights = baryWeight.weight0 + baryWeight.weight1 + baryWeight.weight2 + baryWeight.weight3;
			if (totalWeights > 0)
			{
				baryWeight.weight0 = baryWeight.weight0 / totalWeights;
				baryWeight.weight1 = baryWeight.weight1 / totalWeights;
				baryWeight.weight2 = baryWeight.weight2 / totalWeights;
				baryWeight.weight3 = baryWeight.weight3 / totalWeights;
			}
			else
			{
				baryWeight.weight0 = 1;
				baryWeight.boneIndex0 = 0;
			}
			//baryWeight.weight0 = uniqueBones


			return baryWeight;
		}
		public static Vector3 ClosestPointOnPlane(Vector3 point, Vector3 normal, float distance)
		{
			var pointToPlaneDistance = Vector3.Dot(normal, point) + distance;
			return point - (normal * pointToPlaneDistance);
		}
		public static Vector3 BarycentricCoords(Vector3 aV1, Vector3 aV2, Vector3 aV3, Vector3 aP)
		{
			Plane pln = new Plane(aV1, aV2, aV3);
			aP = ClosestPointOnPlane(aP, pln.normal, pln.distance);
