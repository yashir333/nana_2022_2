	...
	public class SplineEditor : Editor
	{
		...
		static void CreateSpline()
		{
			if (splineStoreData.Data.Count > 2 && splineStoreData.Data[0] && splineStoreData.Data[1] && splineStoreData.Data[2])
			{
				...
				//spline.UpDirection = (spline._splineCTRLS[spline._splineCTRLS.Count - 1].position - spline._splineCTRLS[0].position).normalized;
				//spline.UpDirection = Vector3.Cross(spline.UpDirection, spline._splineCTRLS[0].forward);
				spline.Create();
				spline.UpDirection = Vector3.up;
				Vector3 spineDir = (spline.bones[spline.bones.Count - 1].transform.position - spline.bones[0].transform.position).normalized;
				if(Mathf.Abs(Vector3.Dot(spineDir, Vector3.up))>0.75f)
					spline.UpDirection = Vector3.forward;

				float splineLength = (spline._splineCTRLS[spline._splineCTRLS.Count - 1].position - spline._splineCTRLS[0].position).magnitude;
				/*foreach (Transform _splineCTRL in spline._splineCTRLS)
				{
					if(_splineCTRL.GetComponent<Spline>()!=null)
						_splineCTRL.GetComponent<Spline>().HandleSize = new Vector3(splineLength, splineLength, splineLength) / 10f;
				}*/
				...
				foreach (GameObject bone in spline.bones)
				{
					bone.GetComponent<Bone>().Radius = splineLength/200f;
