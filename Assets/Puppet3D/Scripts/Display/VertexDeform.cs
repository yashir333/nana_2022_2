...
namespace Puppet3D
{
	...
	public class VertexDeform : MonoBehaviour
	{
		...
		void Run()
		{
			...
			Transform parentTrans = _parent.transform;
			if (testGo.Contains(this.gameObject))
			{
				...
				//if (Active)
				{
					...
					_parent.verts[CurrentIndex] = parentTrans.InverseTransformPoint(_parent.Deforms[CurrentIndex].transform.position);
					...
					while (indexQueue.Count > 0)
					{
						...
						if (!_parent.Deforms[workingIndex].Editted)
						{
							...
							if (dist < _falloff)
							{
								...
								_parent.verts[workingIndex] = parentTrans.InverseTransformPoint(_parent.Deforms[workingIndex].transform.position);
