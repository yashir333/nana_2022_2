...
namespace BeautifyHDRP {
    ...
    public sealed class Beautify : CustomPostProcessVolumeComponent, IPostProcessComponent {
        ...
        public BoolParameter compareSameSide = new BoolParameter(true);
        ...
        RTHandle rtBokeh;
        RTHandle rtBlurTemp, rtBlurTemp2, rtBlurTempDownscaling;
        ...
        void PrepareRT(ref RTHandle rt, HDCamera camera, float downscale, UnityEngine.Experimental.Rendering.GraphicsFormat colorFormat, FilterMode filterMode = FilterMode.Bilinear) {
            int w = Mathf.Max(1, (int)(camera.actualWidth / downscale));
            int h = Mathf.Max(1, (int)(camera.actualHeight / downscale));
            PrepareRT(ref rt, w, h, colorFormat, filterMode);
        }

        void PrepareRT(ref RTHandle rt, int w, int h, UnityEngine.Experimental.Rendering.GraphicsFormat colorFormat, FilterMode filterMode = FilterMode.Bilinear) {
        ...
        public override void Render(CommandBuffer cmd, HDCamera camera, RTHandle source, RTHandle destination) {
            ...
            if (requestScreenCapture && cam != null && cam.cameraType == captureCameraType) {
                ...
                FullScreenBlit(cmd, source, rtCapture, (int)ShaderParams.Pass.CopyExact);
            ...
            RTHandle dest;
            if (compareMode.value && camera.camera.cameraType == CameraType.Game) {
                PrepareRT(ref rtCompare, camera, 1, rtSource.rt.graphicsFormat, FilterMode.Point);
                cmd.SetGlobalTexture(ShaderParams.compareTex, rtCompare);
                float angle = compareSameSide.value ? Mathf.PI * 0.5f : compareLineAngle.value;
                m_Material.SetVector(ShaderParams.compareParams, new Vector4(Mathf.Cos(angle), Mathf.Sin(angle), compareSameSide.value ? comparePanning.value : -10, compareLineWidth.value));
                dest = rtCompare;
            } else {
                dest = destination;
            }
            ...
            if (pixelScale != 1f) {
                ...
                FullScreenBlit(cmd, rtDownscaled, dest, (int)ShaderParams.Pass.CopyExact);
            } else {
                HDUtils.DrawFullScreen(cmd, m_Material, dest, shaderPassId: (int)ShaderParams.Pass.Beautify);
            }

            ApplyFinalBlur(cmd, camera, dest);

            if (compareMode.value) {
                HDUtils.DrawFullScreen(cmd, m_Material, destination, shaderPassId: (int)ShaderParams.Pass.Compare);
            }
        ...
        void DoLUT() {
            Texture lutTex = lut.value;
            bool hasLut = lutTex != null;
            bool hasLut3D = hasLut && lutTex is Texture3D;
            if (lutIntensity.value > 0 && (hasLut || hasLut3D)) {
                if (hasLut3D) {
                    m_Material.SetTexture(ShaderParams.lut3DTexture, lutTex);
                    m_Material.SetVector(ShaderParams.lut3DParams, new Vector4(1f / lutTex.width, lutTex.width - 1f, 0, 0));
                    m_Material.EnableKeyword(ShaderParams.SKW_LUT3D);
                } else {
        ...
        void ApplyFinalBlur(CommandBuffer cmd, HDCamera camera, RTHandle source) {

            float blurIntensity = this.blurIntensity.value;
            if (blurIntensity <= 0 || camera.camera.cameraType != CameraType.Game) return;

            int size;
            Vector2Int sourceSize = new Vector2Int(camera.actualWidth, camera.actualHeight);

            if (blurIntensity < 1f) {
                size = (int)Mathf.Lerp(sourceSize.x, 512, blurIntensity);
            } else {
                size = (int)(512 / blurIntensity);
            }
            float aspectRatio = (float)sourceSize.y / sourceSize.x;
            int w = size;
            int h = Mathf.Max(1, (int)(size * aspectRatio));

            float ratio = (float)sourceSize.x / size;
            float blurScale = blurIntensity > 1f ? 1f : blurIntensity;

            PrepareRT(ref rtBlurTempDownscaling, w, h, source.rt.graphicsFormat);
            PrepareRT(ref rtBlurTemp, w, h, source.rt.graphicsFormat);
            PrepareRT(ref rtBlurTemp2, w, h, source.rt.graphicsFormat);

            cmd.SetGlobalFloat(ShaderParams.blurScale, blurScale * ratio);
            FullScreenBlit(cmd, source, rtBlurTempDownscaling, compareMode.value ? (int)ShaderParams.Pass.BlurHoriz : (int)ShaderParams.Pass.BlurHorizFirstPass);
            cmd.SetGlobalFloat(ShaderParams.blurScale, blurScale);
            FullScreenBlit(cmd, rtBlurTempDownscaling, rtBlurTemp, (int)ShaderParams.Pass.BlurVert);

            BlurThis(cmd, rtBlurTemp, blurScale);
            BlurThis(cmd, rtBlurTemp, blurScale);
            BlurThis(cmd, rtBlurTemp, blurScale);

            FullScreenBlit(cmd, rtBlurTemp, source, (int)ShaderParams.Pass.Copy);
        }


        void BlurThis(CommandBuffer cmd, RTHandle rt, float blurScale = 1f) {
            cmd.SetGlobalFloat(ShaderParams.blurScale, blurScale);
            FullScreenBlit(cmd, rt, rtBlurTemp2, (int)ShaderParams.Pass.BlurHoriz);
            FullScreenBlit(cmd, rtBlurTemp2, rt, (int)ShaderParams.Pass.BlurVert);
        }
